#!/usr/bin/env pwsh
<#
Initialize CFIPros roadmap from template.
Creates .specify/memory/roadmap.md if it doesn't exist.
...
$memoryDir = Join-Path -Path $repoRoot -ChildPath '.specify' | Join-Path -ChildPath 'memory'
$roadmapPath = Join-Path -Path $memoryDir -ChildPath 'roadmap.md'
$templatePath = Join-Path -Path $repoRoot -ChildPath '.specify' | Join-Path -ChildPath 'templates' | Join-Path -ChildPath 'roadmap-template.md'
...
if ($roadmapExists) {
    if ($Json) {
        Write-Output '{"exists": true, "path": ".specify/memory/roadmap.md", "created": false}'
    } else {
        Write-Information "✓ Roadmap already exists at: .specify/memory/roadmap.md" -InformationAction Continue
    }
    exit 0
}
...
if (-not $templateExists) {
    $errorMsg = "Template not found at: .specify/templates/roadmap-template.md"
    if ($Json) {
...
if ($Json) {
    Write-Output '{"exists": false, "path": ".specify/memory/roadmap.md", "created": true}'
} else {
    Write-Information "✓ Created roadmap at: .specify/memory/roadmap.md" -InformationAction Continue
