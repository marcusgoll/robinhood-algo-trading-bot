...
function Get-CurrentBranch {
    ...
    if ($env:SPECIFY_FEATURE) {
        $nf = Convert-ToFeatureNameSlug -Name $env:SPECIFY_FEATURE
        return $nf
    }
    ...
    if (Test-HasGit) {
        ...
        try {
            ...
            if ($LASTEXITCODE -eq 0 -and $sha) {
                Write-Warning "[specify] Detached HEAD at $sha; using commit id as branch label"
                return $sha.Trim()
            }
        }
        catch {}
...
function Test-FeatureBranch {
    ...
    if (-not $HasGit) {
        Write-Warning "[specify] Git not detected; skipping branch validation"
        return $true
    }
...
function Get-FeaturePathsEnv {
    ...
    [PSCustomObject]@{
        ...
        MEMORY_DIR          = Join-Path -Path $repoRoot -ChildPath '.specify\memory'
        CONSTITUTION        = Join-Path -Path $repoRoot -ChildPath '.specify\memory\constitution.md'
        ROADMAP             = Join-Path -Path $repoRoot -ChildPath '.specify\memory\roadmap.md'
        DESIGN_INSPIRATIONS = Join-Path -Path $repoRoot -ChildPath '.specify\memory\design-inspirations.md'
    }
}
