...
<#
.SYNOPSIS
    Automated task tracking for CFIPros implementation workflow
...
function Get-FeatureDirectory {
    ...
    $specsDir = Join-Path $PWD ".specify" "memory" "specs"
    if (-not (Test-Path $specsDir)) {
        throw "No specs directory found. Run /specify first."
    }
...
function Get-RecommendedAgent {
    ...
    foreach ($path in $FilePaths) {
        if ($path -match "apps/api") { return "cfipros-backend-dev" }
        if ($path -match "apps/web") { return "cfipros-frontend-shipper" }
        if ($path -match "contracts/") { return "cfipros-contracts-sdk" }
        if ($path -match "migrations|alembic") { return "cfipros-database-architect" }
        if ($path -match "\.github/workflows") { return "cfipros-ci-cd-release" }
    }

    # Analyze description for task type
    if ($Description -match "test|spec") { return "cfipros-qa-test" }
    if ($Description -match "debug|fix|error") { return "cfipros-debugger" }
    if ($Description -match "coverage") { return "tdd-coverage-enhancer" }
    if ($Description -match "database|schema|migration") { return "cfipros-database-architect" }
