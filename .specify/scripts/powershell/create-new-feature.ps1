...
if ($hasGit) {
    ...
    Write-Warning "[specify] Git not detected; skipping branch creation (planned: $branchName)"
}
...
# Candidate spec templates (prefer .specify override)
$specTemplateCandidates = @(
    (Join-Path -Path $repoRoot -ChildPath '.specify/templates/spec-template.md'),
...
# Set SPECIFY_FEATURE for current session
