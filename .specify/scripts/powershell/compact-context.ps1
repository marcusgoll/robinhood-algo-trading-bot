...
# --- Reset token baseline after compaction --------------------------------
$baselineFile = Join-Path -Path $FeatureDir -ChildPath ".token-baseline"
if (Test-Path -LiteralPath $baselineFile -PathType Leaf) {
    # Query current cumulative total from ccusage
    $projectPath = (Get-Location).Path
    $usageJson = npx -y ccusage@latest session --project $projectPath --json 2>$null | ConvertFrom-Json
    if ($usageJson -and $usageJson.sessions -and $usageJson.sessions.Count -gt 0) {
        $cumulativeInput = $usageJson.sessions[0].inputTokens
        $cumulativeOutput = $usageJson.sessions[0].outputTokens
        $cumulativeTotal = $cumulativeInput + $cumulativeOutput

        # Reset baseline to current cumulative total
        Set-Content -LiteralPath $baselineFile -Value $cumulativeTotal -NoNewline
    }
}
