...
if [ -z "$ARGUMENTS" ]; then
  ...
  if [ -z "$PR_NUMBER" ]; then
    echo "Usage: /checks pr [number]"
    echo "   or: /checks [number]"
    echo ""
...
if [ "$PENDING" -gt 0 ]; then
  ...
  echo ""
  echo "Wait for checks to complete, then re-run /checks"
  exit 0
fi
...
---
🤖 Generated by /checks" 2>/dev/null || echo "Failed to post comment (check gh auth)"
...
Applied ESLint/Ruff fixes via /checks
...
---
🤖 Generated by /checks" 2>/dev/null || echo "Comment post failed"
    else
...
**Next**: Wait for delegated fixes to complete, then re-run \`/checks pr $PR_NUMBER\`
...
if [ "$GATES_PASSED" -eq "$GATES_TOTAL" ]; then
  ...
  echo "Next: Wait for delegated fixes, then re-run /checks"
fi
