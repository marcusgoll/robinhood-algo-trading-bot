...
if ! grep -q "Status: ✅ Ready for Production" "$VALIDATION_REPORT"; then
  ...
  echo "Once all checks pass, run /ship-prod again"
  exit 1
fi
...
if git rev-parse "$NEXT_VERSION" >/dev/null 2>&1; then
  ...
  case "$TAG_OPTION" in
    ...
    2)
      ...
      ;;
    3)
      echo "Please re-run /ship-prod and choose different version"
      exit 1
      ;;
  esac
else
...
---
*Generated by /ship-prod command*
EOF
...
Once all checks pass (✅ Ready for Production), run /ship-prod.
```
